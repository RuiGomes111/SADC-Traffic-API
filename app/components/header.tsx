...
export default function Header() {
  ...
  const linkStyles =
    "flex items-center px-4 py-2 md:h-12 rounded-xl font-bold transition-colors duration-200 ease-in-out";

  const activeStyles = "bg-blue-600 text-white shadow-md";
  const inactiveStyles =
    "text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-blue-500";
    ...
    <header className="w-full h-20 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 flex items-center px-4 md:px-10 fixed top-0 z-50">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="leading-tight">
          <h1 className="text-sm md:text-lg font-bold text-blue-500 whitespace-nowrap">
            SADC Traffic API
          </h1>
          <span className="hidden sm:block text-[10px] md:text-xs text-zinc-500">
      ...
      <nav
        className={`
          absolute md:relative top-20 md:top-0 left-0 w-full md:w-auto
          bg-white dark:bg-zinc-900 md:bg-transparent
          flex-col md:flex-row flex items-center gap-2 p-6 md:p-0 md:ml-auto
          transition-all duration-300 ease-in-out
          ${
            isOpen
              ? "opacity-100 translate-y-0"
              : "opacity-0 -translate-y-10 pointer-events-none md:pointer-events-auto md:opacity-100 md:translate-y-0"
          }
        `}
      >
            ...
            <li key={link.href} className="w-full md:w-auto">
              <Link
                ...
                className={`${linkStyles} ${
                  active === link.href ? activeStyles : inactiveStyles
                }`}
              >
